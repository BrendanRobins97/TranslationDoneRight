    ...
    public class SimilarityReviewWindow : EditorWindow
    {
        ...
        private void LoadExistingGroups()
        {
            ...
            foreach (var textGroup in groupedTexts)
            {
                ...
                var metadata = TranslationMetaDataProvider.Metadata.GetGroupMetadata(textGroup);
        ...
        private void OnGUI()
        {
            ...
            foreach (var group in similarityGroups)
            {
                ...
                var metadata = TranslationMetaDataProvider.Metadata.GetGroupMetadata(group.Texts);
                if (metadata != null)
                {
                    EditorGUILayout.BeginHorizontal();
                ...
                foreach (var text in group.Texts)
                {
                    ...
                    {
                        var sources = TranslationMetaDataProvider.Metadata.GetSources(text);
                        if (sources.Count > 0)
                        {
            ...
            foreach (var group in similarityGroups)
            {
                ...
                if (tempSelections.TryGetValue(groupKey, out var status))
                {
                    if (!status.isAccepted)
                    {
                        ...
                        TranslationMetaDataProvider.Metadata.SetGroupMetadata(
                            group.Texts,
                            group.Reason,
                    ...
                    else
                    {
                        ...
                        TranslationMetaDataProvider.Metadata.ClearGroupMetadata(group.Texts);
                        hasChanges = true;
                    }
                }
            }
