...
namespace Translations
{
    public partial class TranslationsEditorWindow
    {
        ...
        private void DrawKeysList()
        {
            ...
            if (filteredKeysList.Count > 0)
            {
                ...
                for (int i = startIndex; i < endIndex; i++)
                {
                    ...
                    if (TranslationMetaDataProvider.Metadata.IsMissingText(key))
                    {
                        EditorGUI.LabelField(missingLabelRect, "missing", missingLabelStyle);
                    }
                    if (TranslationMetaDataProvider.Metadata.IsNewText(key))
                    {
                        EditorGUI.LabelField(newLabelRect, "new", newLabelStyle);
                    }
                    if (TranslationMetaDataProvider.Metadata.IsRecentText(key))
                    {
        ...
        private void UpdateFilteredKeys()
        {
            ...
            if (showNewOnly)
            {
                initialKeys = initialKeys.Where(k => TranslationMetaDataProvider.Metadata.IsNewText(k));
            }
            
            // Apply Missing state filter if enabled
            if (showMissingStateOnly)
            {
                initialKeys = initialKeys.Where(k => TranslationMetaDataProvider.Metadata.IsMissingText(k));
            }
        ...
        private void DrawTranslationsForKey()
        {
            ...
            if (showSourceInformation)
            {
                ...
                if (selectedKeys.Count == 1)
                {
                    var sources = TranslationMetaDataProvider.Metadata.GetSources(selectedKeys.First());
                    if (sources.Count > 0)
                    {
                ...
                if (showSourceInformation)
                {
                    ...
                    var context = TranslationMetaDataProvider.Metadata.GetContext(referenceKey);
                    
                    // Context categories
                    foreach (string key in TranslationMetaDataProvider.Metadata.TextCategories.Keys)
                    {
                        ...
                        if (!TranslationMetaDataProvider.Metadata.TextCategories.ContainsKey(key))
                        {
                            TranslationMetaDataProvider.Metadata.TextCategories[key] = new List<string>();
                        }
                        string newValue = categoryDropdown.Draw(key, selectedKeys.First().GetHashCode(), context[key], TranslationMetaDataProvider.Metadata.TextCategories[key], translationData, (newCategory) => {
                        if (newCategory != oldCategory)
                        {
                            TranslationMetaDataProvider.Metadata.UpdateTextCategory(key, oldCategory, newCategory);
                        }
                        });

                        if (newValue != oldCategory)
                        {
                            TranslationMetaDataProvider.Metadata.UpdateContext(selectedKeys.First(), key, newValue);
                            foreach (var selectedKey in selectedKeys)
                            {
                                TranslationMetaDataProvider.Metadata.UpdateContext(selectedKey, key, newValue);
                            }
                    ...
                    if (showCategoryManagement)
                    {
                        ...
                        using (new EditorGUILayout.HorizontalScope())
                        {
                            ...
                            {
                                if (!TranslationMetaDataProvider.Metadata.TextCategories.ContainsKey(newCategoryName))
                                {
                                    TranslationMetaDataProvider.Metadata.AddCategory(newCategoryName, new CategoryTemplate { 
                        ...
                        foreach (var category in TranslationMetaDataProvider.Metadata.TextCategories.Keys)
                        {
                            ...
                            // Format editor
                            if (TranslationMetaDataProvider.Metadata.CategoryTemplates.TryGetValue(category, out var template))
                            {
                                EditorGUI.BeginChangeCheck();
                                ...
                                if (EditorGUI.EndChangeCheck())
                                {
                                    ...
                                    TranslationMetaDataProvider.Metadata.UpdateCategoryTemplate(category, template);
                                    EditorUtility.SetDirty(translationData);
                                }
                            }
                        ...
                        {
                            TranslationMetaDataProvider.Metadata.TextCategories.Remove(category);
                            TranslationMetaDataProvider.Metadata.CategoryTemplates.Remove(category);
                            
                            // Apply to all selected keys
                            foreach (var selectedKey in selectedKeys)
                            {
                                var keyContext = TranslationMetaDataProvider.Metadata.GetContext(selectedKey);
                                keyContext.Remove(category);
                                TranslationMetaDataProvider.Metadata.SetContext(selectedKey, keyContext);
                            }
                    ...
                    if (EditorGUI.EndChangeCheck() && (hasSameManualContext || selectedKeys.Count == 1))
                    {
                        ...
                        foreach (var selectedKey in selectedKeys)
                        {
                            TranslationMetaDataProvider.Metadata.UpdateContext(selectedKey, "Manual", newManualContext);
                        }
                        EditorUtility.SetDirty(translationData);
                    }
                    ...
                    if (selectedKeys.Count == 1)
                    {
                        string translationContext = TranslationMetaDataProvider.Metadata.GetTranslationContext(selectedKeys.First());
            ...
            for (int i = 0; i < translationData.supportedLanguages.Count - 1; i++)
            {
                ...
                if (!string.IsNullOrEmpty(deeplApiKey))
                {
                    ...
                    {
                        context = TranslationMetaDataProvider.Metadata.GetTranslationContext(selectedKeys.First());
                    }
                ...
                if (GUILayout.Button(new GUIContent("Translate", tooltipContent), GUILayout.Width(70)))
                {
                    ...
                    foreach (var key in selectedKeys)
                    {
                        ...
                        string context = includeContextInTranslation && selectedKeys.Count == 1 
                            ? TranslationMetaDataProvider.Metadata.GetTranslationContext(key)
                            : "";
                            string assetPath = AssetDatabase.GUIDToAssetPath(assetRef.AssetGUID);
                        LanguageData languageData = AssetDatabase.LoadAssetAtPath<LanguageData>(assetPath);
        ...
        private bool AllSelectedKeysHaveSameContextValue(string category, out string commonValue)
        {
            ...
            var firstContext = TranslationMetaDataProvider.Metadata.GetContext(selectedKeys.First());
            ...
            {
                var keyContext = TranslationMetaDataProvider.Metadata.GetContext(key);
                string value = "";
