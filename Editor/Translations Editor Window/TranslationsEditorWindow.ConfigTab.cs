    ...
    public partial class TranslationsEditorWindow
    {
        ...
        private void DrawDeepLConfigSection()
        {
            ...
            if (TranslationMetaDataProvider.Metadata != null)
            {
                ...
                foreach (var mapping in TranslationMetaDataProvider.Metadata.CustomLanguageMappings.ToList())
                {
                    EditorGUILayout.BeginHorizontal();
                    ...
                    if (newLangIndex != langIndex || newCodeIndex != codeIndex)
                    {
                        if (newLangIndex != langIndex)
                        {
                            ...
                            TranslationMetaDataProvider.Metadata.CustomLanguageMappings[newLang] = mapping.Value;
                        }
                        if (newCodeIndex != codeIndex)
                        {
                            TranslationMetaDataProvider.Metadata.CustomLanguageMappings[mapping.Key] = deeplCodes[newCodeIndex];
                        }
                ...
                {
                    TranslationMetaDataProvider.Metadata.CustomLanguageMappings.Remove(key);
                    EditorUtility.SetDirty(translationData);
                }
                ...
                if (GUILayout.Button("Add New Mapping"))
                {
                    ...
                    string newLang = translationData.supportedLanguages
                        .FirstOrDefault(l => !TranslationMetaDataProvider.Metadata.CustomLanguageMappings.ContainsKey(l));
                    
                    if (!string.IsNullOrEmpty(newLang))
                    {
                        TranslationMetaDataProvider.Metadata.CustomLanguageMappings[newLang] = "EN"; // Default to EN
                        EditorUtility.SetDirty(translationData);
                    }
                }
        ...
        public async Task TranslateAllLanguagesForKey(string key)
        {
            ...
            try
            {
                ...
                string context = includeContextInTranslation ? TranslationMetaDataProvider.Metadata.GetTranslationContext(key) : "";
